...
router.put('/profile/profileUpdate',upload.single('image'),async (req, res) => {

  const updatedDetails = req.body;
  const image= req.file.path;
  // console.log(file,"file")

console.log(image,"file upd")
   console.log(updatedDetails,"reqboody")
  try {
    // Find the user with the provided email in your database
    const user = await db.User.findOne({
    ...
    user.title = updatedDetails.title;
    user.contact = updatedDetails.contact;
    user.country = updatedDetails.country;
    user.state = updatedDetails.state;
    user.street = updatedDetails.street;
    user.city = updatedDetails.city;
    user.postalcode = updatedDetails.postalcode;
    user.language = updatedDetails.language;
    user.timezone = updatedDetails.timezone;
    user.profilepic = image;


    await user.save();
   console.log(200,"status")
   return res.status(200).send('User details updated successfully');// User details updated successfully
  } catch (error) {
